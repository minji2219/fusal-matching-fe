...
const Modal = ({
  ...
  const oneTeamMatch = async () => {
    try {
      const result = await axios.post(
        "https://6f2b-121-147-100-85.ngrok-free.app/matching/apply",
        {
  ...
  const firstMatch = async () => {
    try {
      const result = await axios.post(
        "https://6f2b-121-147-100-85.ngrok-free.app/matching/create",
        {
          team: idData,
          stadium: field.stadiumId,
          field: field.id,
  ...
  return (
    <div className="modal" style={{ display: { display } }}>
      <div className="modal_head" style={{ backgroundColor: `${titleColor}` }}>
        ...
        <span className="material-symbols-outlined" onClick={cancel}>
          cancel
        </span>
      </div>
